...
pub fn search_fts5(
    ...
    println!("[FTS5] Searching for: '{}' (max: {}, offset: {})", fts_query, max_results, offset);
    ...
    let rows_result = if let Some(file_path) = file_path_filter {
        stmt.query(params![&fts_query, file_path, max_results as i64, offset as i64])
    } else {
    ...
    println!("[FTS5] Found {} results in {:?}", results.len(), start.elapsed());

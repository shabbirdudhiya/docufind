...
fn parse_styles(archive: &mut ZipArchive<fs::File>) -> std::collections::HashMap<String, StyleInfo> {
                    ...
                    Ok(XmlEvent::EndElement { name }) => {
                        if name.local_name == "style" && in_style {
                            if !current_style_id.is_empty() {
                                let heading_level = detect_heading_level(&current_style_id, &current_style_name);
                                styles.insert(
...
fn parse_document_xml(xml: &str, style_map: &std::collections::HashMap<String, StyleInfo>) -> Vec<ContentSection> {
                ...
                match name.local_name.as_str() {
                    ...
                    "u" => {
                        if in_run {
                            ...
                            let is_disabled = attributes.iter().any(|a| 
                                a.name.local_name == "val" && a.value == "none"
                            );
                            if !is_disabled {
                ...
                match name.local_name.as_str() {
                    ...
                    "p" => {
                        if in_paragraph && !current_runs.is_empty() {
                            ...
                            let combined_content: String = current_runs.iter()
                                .map(|r| r.text.as_str())
                                .collect();
